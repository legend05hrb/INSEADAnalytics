...
**Your Answers here: There are lots of finance related R package in the below link:
https://cran.r-project.org/web/views/Finance.html
I found package "tvm" extremly useful, since it provides functions for time value of money such as cashflows and yield curves, which is foundamental for company valuation
**
<br>
<br>
...
**Your Answers here:
1.In file “dataSet1.R”, line 8
2.We have `r nrow(StockReturns)` days of data, starting from `r rownames(StockReturns)[1]` until `r tail(rownames(StockReturns),1)`.  Here are some basic statistics about the S&P returns:

1. The cumulative returns of the S&P index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
2. The average daily returns of the S&P index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
2. The standard deviation of the daily returns of the S&P index during this period is `r round(100*sd(StockReturns[,2]),3)`%;

Here are returns of the S&P in this period:
...
3.In file "dataSet1.R", line 8, add mytickers such as "GOOG"
**
...
1.number of rows: nrow(transposedData), number of columns: ncol(transposedData)
...
**Your Answers here:
1. change line 9, startDate = "2000-12-29"
**
<br>
<br>
<br>
